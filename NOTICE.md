...
Takt v0.1.55-alpha uses `go.yaml.in/yaml/v3` v3.0.4 for YAML syntax parsing. The module is maintained by the YAML organization at `github.com/yaml/go-yaml` and is distributed under permissive MIT/Apache-2.0 terms. Its source is not vendored into this archive; Go resolves the exact version and checksum from `go.mod`/`go.sum`.
