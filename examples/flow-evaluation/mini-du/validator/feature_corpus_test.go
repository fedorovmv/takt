...
func TestFeatureCorpusManifest(t *testing.T) {
	...
	for i, c := range cases {
		...
		for _, contract := range []string{"Usage: mini-du [-s] [-k|-H] [--] [PATH...]", "1536 bytes as 1.5KiB", "-sH", "Without `-k`, numeric output is also integer kibibytes", "filesystem allocated blocks"} {
