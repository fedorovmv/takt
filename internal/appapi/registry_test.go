...
func TestCanonicalOperationRegistry(t *testing.T) {
	...
	want := []string{
		...
		"run.abandon", "run.answer", "run.artifacts", "run.assessment", "run.attention", "run.cancel", "run.children", "run.events", "run.fork", "run.get", "run.inspect", "run.list", "run.pause", "run.recover", "run.resume", "run.resume_paused", "run.retry", "run.start", "run.stats", "run.status", "run.summary",
