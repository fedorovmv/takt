...
func addFlowNode(record *RunRecord, id string, node store.NodeState) {
	...
	record.Nodes[id] = NodeRecord{Status: node.Status, Attempts: node.Attempts, ProviderAttempts: node.ProviderAttempts, Assistant: node.Assistant, Adapter: node.Adapter, AssistantVersion: node.AssistantVersion, RequestedModel: node.RequestedModel, ResolvedModel: node.ResolvedModel, SessionID: node.SessionID, SessionPath: node.SessionPath, Prompt: node.Prompt, PromptFingerprint: node.PromptFingerprint, Resumed: node.Resumed, ExitCode: node.ExitCode, ErrorCode: node.ErrorCode, Error: node.Error, Feedback: node.Feedback, DiagnosticOutput: node.Output, Stdout: node.Stdout, Stderr: node.Stderr, OutputTruncated: node.OutputTruncated, Usage: node.Usage, Diagnostic: node.Diagnostic, MixedIdentity: mixed, Executions: executions}
}
