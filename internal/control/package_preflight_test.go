...
func TestPackageAdapterPreflightRequiredAndPreferred(t *testing.T) {
	...
	statuses, err := preflightCatalogAdapters(context.Background(), required, cfg)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := preflightCatalogAdapters(context.Background(), missing, cfg); err == nil {
		t.Fatal("expected required capability failure")
	}
	missing.Requirements.Adapters[0].Level = "preferred"
	statuses, err = preflightCatalogAdapters(context.Background(), missing, cfg)
	if err != nil {
		t.Fatal(err)
	}
