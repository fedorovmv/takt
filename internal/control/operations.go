...
func (s *Service) ListRuns(request RunListRequest) ([]RunListEntry, error) {
...
func runListEntry(st store.FS, state *store.RunState) RunListEntry {
...
func (s *Service) Attention() ([]AttentionItem, error) {
...
func (s *Service) Summary(runID string, recursive bool) (*RunSummary, error) {
	st := store.FS{Workspace: s.Workspace}
...
func (s *Service) Pause(runID string) (*PauseResult, error) {
	st := store.FS{Workspace: s.Workspace}
	root, err := st.Load(runID)
	if err != nil {
		return nil, err
...
func (s *Service) ResumePaused(ctx context.Context, runID string, detached bool) (*store.RunState, error) {
	st := store.FS{Workspace: s.Workspace}
...
func (s *Service) Abandon(runID, reason string) (any, error) {
	st := store.FS{Workspace: s.Workspace}
	root, err := st.Load(runID)
	if err != nil {
		return nil, err
	...
	for _, id := range ids {
		...
		if state.Status == store.RunWaiting || state.Status == store.RunPaused {
			...
			state, loadErr = st.Load(id)
			if loadErr == nil {
				runner, runnerErr := runnerForState(state)
				if runnerErr == nil {
...
func runTreeIDs(st store.FS, root *store.RunState) ([]string, error) {
	ids := []string{root.ID}
	seen := map[string]bool{root.ID: true}
	queue := append([]string(nil), root.ChildRunIDs...)
	for len(queue) > 0 {
...
func (s *Service) Retry(ctx context.Context, request RetryRequest) (*store.RunState, error) {
	st := store.FS{Workspace: s.Workspace}
...
func (s *Service) Fork(ctx context.Context, request ForkRequest) (*ForkResult, error) {
	st := store.FS{Workspace: s.Workspace}
	state, err := st.Load(request.RunID)
	if err != nil {
		return nil, err
	}
	if record, planErr := s.resolvePlanRecord("", request.RunID); planErr == nil {
...
func (s *Service) setOwningPlanStatus(runID, status, lastError string) error {
