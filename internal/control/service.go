...
// Service exposes the local Takt control plane without adding a daemon or a
// second persistence model. All durable state remains in the workspace store.
type Service struct {
	Workspace  string
	ConfigPath string
...
func New(workspace, configPath string) (*Service, error) {
	...
	return &Service{Workspace: absWorkspace, ConfigPath: absConfig, launchErrors: map[string]error{}}, nil
}

func (s *Service) ListWorkflows(profileName string) ([]WorkflowListEntry, error) {
...
func (s *Service) DescribeWorkflow(selector string) (*WorkflowDescription, error) {
...
func (s *Service) Start(ctx context.Context, request StartRequest) (*StartResult, error) {
	...
	storeFS := store.FS{Workspace: s.Workspace}
...
func (s *Service) prepareStart(request StartRequest) (*preparedStart, error) {
	selector := strings.TrimSpace(request.Selector)
	if selector == "" {
		return nil, fmt.Errorf("selector is required")
	}
	wfPath, cfgPath, resolved, err := s.resolveWorkflow(selector, request.ConfigPath)
	if err != nil {
		return nil, err
	}
...
func (s *Service) resolveWorkflow(value, configOverride string) (string, string, *profile.Resolved, error) {
	candidate := value
...
func (s *Service) absoluteFromWorkspace(value string) (string, error) {
...
func (s *Service) Cancel(runID, reason string) (any, error) {
...
func (s *Service) Children(runID string) (map[string]any, error) {
	st := store.FS{Workspace: s.Workspace}
...
func (s *Service) Artifacts(runID string, query ArtifactQuery) (map[string]any, error) {
	st := store.FS{Workspace: s.Workspace}
...
func (s *Service) Events(ctx context.Context, runID string, afterRevision uint64, limit int, wait time.Duration) (*EventsResult, error) {
...
func (s *Service) setLaunchError(runID string, err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.launchErrors[runID] = err
}

func (s *Service) launchError(runID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
