...
func (s *Service) BeginHostSession(ctx context.Context, request HostBeginRequest) (*HostBeginResult, error) {
	s.hostMu.Lock()
	...
	if existing, findErr := store.Find(host, hostSessionID); findErr == nil {
		...
		view, viewErr := s.GetPlan(existing.PlanID)
		if viewErr != nil {
			return nil, viewErr
		}
...
func (s *Service) ConfirmHostSession(ctx context.Context, request HostConfirmRequest) (*HostSessionView, error) {
	...
	record, err := s.ExecutePlan(ctx, ExecutePlanRequest{PlanID: session.PlanID, Confirm: request.Confirm, Detached: request.Detached})
	if err != nil {
		return nil, err
	}
	...
	plan, err := s.GetPlan(session.PlanID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) GetHostSession(sessionID string) (*HostSessionView, error) {
	s.hostMu.Lock()
	defer s.hostMu.Unlock()
	store := hostcontrol.Store{Workspace: s.Workspace}
...
func (s *Service) FindHostSession(host, hostSessionID string) (*HostSessionView, error) {
	s.hostMu.Lock()
	defer s.hostMu.Unlock()
	store := hostcontrol.Store{Workspace: s.Workspace}
...
func (s *Service) GuardHostTool(request HostToolGuardRequest) (*HostGuardDecision, error) {
...
func (s *Service) GuardHostCompletion(request HostCompletionGuardRequest) (*HostGuardDecision, error) {
	view, err := s.GetHostSession(request.SessionID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) ReleaseHostSession(sessionID string) (*hostcontrol.Session, error) {
	s.hostMu.Lock()
	defer s.hostMu.Unlock()
	store := hostcontrol.Store{Workspace: s.Workspace}
	session, err := store.Load(sessionID)
...
func (s *Service) refreshHostSession(store hostcontrol.Store, session *hostcontrol.Session) (*HostSessionView, error) {
	plan, err := s.GetPlan(session.PlanID)
	if err != nil {
		return nil, err
	}
