...
func PreflightAdapters(ctx context.Context, catalog *Catalog, cfg *spec.Config, resolver domainadapter.Resolver) ([]PreflightStatus, error) {
	if catalog == nil || len(catalog.Requirements.Adapters) == 0 {
		return nil, nil
	}
	statuses := make([]PreflightStatus, 0, len(catalog.Requirements.Adapters))
	for _, req := range catalog.Requirements.Adapters {
		level := req.Level
		if level == "" {
			level = "required"
		}
		st := PreflightStatus{Name: req.Name, Domain: req.Domain, Level: level}
