...
func serveMCP(domain string) {
	...
	for scanner.Scan() {
		...
		switch method {
		case "initialize":
			rpc(id, map[string]any{"protocolVersion": "2025-11-25", "capabilities": map[string]any{"tools": map[string]any{}}, "serverInfo": map[string]any{"name": "takt-fake-domain-adapter", "version": "0.1.43-alpha"}})
		case "tools/list":
