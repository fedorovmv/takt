...
func learnCmd(args []string) error {
	...
	case "scan":
		...
		patterns, err := (learning.Manager{Workspace: *workspace}).Scan(ctx, *minRuns)
		if err != nil {
			return err
		}
	...
	case "list":
		...
		proposals, err := (learning.Manager{Workspace: *workspace}).List()
		if err != nil {
			return err
		}
	...
	case "get":
		...
		proposal, err := (learning.Manager{Workspace: *workspace}).Load(fs.Arg(0))
		if err != nil {
			return err
		}
	...
	case "propose":
		...
		proposal, err := (learning.Manager{Workspace: *workspace}).Propose(ctx, learning.ProposeRequest{
	...
	case "review":
		...
		proposal, err := (learning.Manager{Workspace: *workspace}).Review(fs.Arg(0), *decision, *reason)
		if err != nil {
			return err
		}
		return printResult(*jsonOut, proposal)
	case "evaluate":
		...
		proposal, err := (learning.Manager{Workspace: *workspace}).Evaluate(fs.Arg(0), *report)
		if err != nil {
			return err
		}
		return printResult(*jsonOut, proposal)
	case "stage":
		...
		proposal, err := (learning.Manager{Workspace: *workspace}).Stage(fs.Arg(0))
		if err != nil {
			return err
		}
		return printResult(*jsonOut, proposal)
