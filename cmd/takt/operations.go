...
func runsCmd(args []string) error {
	...
	request := control.RunListRequest{Status: *status, ActiveOnly: *active, AttentionOnly: *attention, RootOnly: *rootOnly, Limit: *limit}
	var result []control.RunListEntry
	if *useDaemon {
		...
		result, err = service.ListRuns(request)
		if err != nil {
			return err
		}
	}
...
func attentionCmd(args []string) error {
	...
	var result []control.AttentionItem
	if *useDaemon {
		client, err := daemon.NewClient(*workspace, *socket)
		if err != nil {
			return err
...
func runPauseCmd(args []string) error {
	...
	var result control.PauseResult
	if *useDaemon {
		client, err := daemon.NewClient(*workspace, *socket)
		if err != nil {
			return err
...
func runResumePausedCmd(args []string) error {
	...
	if *useDaemon {
		...
		value, err := service.ResumePaused(context.Background(), fs.Arg(0), false)
		if err != nil {
			return err
		}
		result = *value
...
func runRetryCmd(args []string) error {
	...
	request := control.RetryRequest{RunID: fs.Arg(0), NodeID: *node}
	var result store.RunState
	if *useDaemon {
		client, err := daemon.NewClient(*workspace, *socket)
		if err != nil {
		...
		value, err := service.Retry(context.Background(), request)
		if err != nil {
			return err
		}
		result = *value
...
func runForkCmd(args []string) error {
	...
	request := control.ForkRequest{RunID: fs.Arg(0), Input: *input}
	var result control.ForkResult
	if *useDaemon {
		client, err := daemon.NewClient(*workspace, *socket)
		if err != nil {
			return err
		...
		value, err := service.Fork(context.Background(), request)
		if err != nil {
			return err
		}
		result = *value
...
func runRecoverCmd(args []string) error {
	...
	var result control.RecoverResult
	if *useDaemon {
		client, err := daemon.NewClient(*workspace, *socket)
		if err != nil {
			return err
...
func runWatchCmd(args []string) error {
	...
	var revision uint64
	for {
		var events control.EventsResult
		...
		if *useDaemon {
			...
			events = *value
			current, err := service.GetRun(runID)
			if err != nil {
				return err
			}
			state = *current
		...
		if terminalRunState(state.Status) || state.Status == store.RunWaiting || state.Status == store.RunPaused {
			if !*jsonOut {
