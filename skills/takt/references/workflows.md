...
`when` — только gate: `==`, `!=`, `&&`, `||` по `nodes.*`/`inputs.*`. Скобки, функции, арифметику и другие вычисления выносите в `script`/`command`/`prompt`, а `when` применяйте к их output. Правило языка: **YAML координирует, код вычисляет, агент принимает решения**.
