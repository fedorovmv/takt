...
Takt не заявляет бинарную или YAML-совместимость с Archon. Цель — перенести полезные процессы и сохранить знакомую модель DAG без второго runtime. В `v0.1.32-alpha` встроенный профиль `code` 0.9.0 содержит 19 процессов, соответствующих стандартному каталогу Archon, и умный роутер с no-tool policy как корневой Run и выбранный процесс как governed child Run.
