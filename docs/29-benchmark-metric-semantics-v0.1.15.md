...
`NodeState` сохраняет агрегированные поля последней фактической попытки для совместимости, а также массив `executions`. Переход `attempts_exhausted` не обнуляет Session ID, `resumed` и output последней попытки. Одна запись соответствует одному фактическому вызову действия узла и содержит:
