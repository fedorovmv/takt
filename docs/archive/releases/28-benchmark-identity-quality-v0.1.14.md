...
В `v0.1.14-alpha` эти поля отражали последнюю попытку узла. Полная атрибуция retry по отдельным execution identity добавлена в `v0.1.15-alpha` и описана в `29-benchmark-metric-semantics-v0.1.15.md`.
...
- стоимость и амортизированную end-to-end длительность на один корректный результат;
- diagnostics по severity и code;
- распределение assistant, requested model и resolved model.

Стоимость и амортизированная длительность учитывают весь прогон benchmark, включая неуспешные задания. Поле `duration_per_valid_ms` из этой версии было переименовано в `v0.1.15-alpha`, поскольку оно не являлось фактическим time-to-valid.
