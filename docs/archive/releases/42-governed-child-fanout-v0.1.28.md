...
- `one_success` и `all_success` пока не завершают группу досрочно и не отменяют/останавливают остальные элементы автоматически;
- одинаковые канонические элементы отклоняются по умолчанию; повторный запуск требует `allow_duplicates: true`;
