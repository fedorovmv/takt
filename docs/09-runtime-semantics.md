...
`subworkflow` и последовательный `foreach` разворачиваются до запуска в обычные DAG-узлы. Тот же компилятор работает внутри `loop_group`. Runtime сохраняет один scheduler, один Run и одну модель persistence; дочерние действия используют общую семантику hooks, retry, approval и ошибок.
...
`foreach` выполняет элементы последовательно и собирает output каждой итерации в упорядоченный JSON-массив. Параллельное выполнение требует отдельной scheduler-семантики и в `v1alpha1` не реализовано.
