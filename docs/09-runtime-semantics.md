...
Статус документа: целевой контракт v0.2. Семантика отказов, DAG, `loop_group`, approval, fingerprints, persistence и per-attempt execution identity уже реализована в `v0.1.18-alpha`. Оставшиеся отличия перечислены в `05-implementation-status.md`.
...
`subworkflow` и последовательный `foreach` разворачиваются до запуска в обычные DAG-узлы. Runtime не создаёт второй scheduler и не хранит отдельный скрытый Run. Поэтому дочерние `command`, `prompt`, `bash`, hooks, retry, approval и статусы используют существующую семантику.

Контейнер сохраняет публичный ID, а скомпилированные дочерние узлы получают стабильные ID с `__`. Approval внутри subworkflow останавливает родительский Run и возобновляется обычной командой `takt answer` по отображённому namespaced node ID.

Fingerprint workflow включает исходный родительский файл и каноническое скомпилированное определение. Изменение подключённого workflow, его локальной Markdown-команды, входов или `foreach.items` блокирует resume ранее начатого Run.

Текущий `foreach` принимает только статический явный список и выполняет элементы последовательно. Markdown остаётся пользовательским документом и не преобразуется runtime в task AST. Динамические источники данных относятся к будущему контракту input adapters.
