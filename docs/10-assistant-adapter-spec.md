...
Pi использует `~/.pi/agent/settings.json` (глобальный) и `.pi/settings.json`
(проектный; он имеет приоритет), а в интерактивном режиме настройки доступны
через `/settings`. Для изолированного рабочего каталога оценки тот же нативный
JSON задаётся непосредственно в Config без переименования ключей:
...
Эквивалентный Pi JSON имеет вид:
...
`assistants.<name>.settings` разрешён только для `type: pi` и является полным
объектом настроек Pi, а не отдельной схемой параметров Takt. Это позволяет
добавлять поддерживаемые ключи Pi без новых полей Config. `max_output_bytes`
остаётся собственным лимитом адаптера Takt и задаётся рядом с `settings`.

Оценка production-flow и `eval analyze` создают из этого блока
изолированный `.pi/settings.json`, после чего принудительно отключают повторы
агента и обнуляют число повторов SDK провайдера. Ключи повторов, которыми владеет
контур оценки, имеют приоритет над заданными значениями; остальные ключи Pi
сохраняются. Переопределение действует только в скопированном рабочем каталоге
оценки: Pi возвращает первую типизированную ошибку провайдера, а Takt владеет
сохраняемым повтором той же сессии. Обычные запуски вне оценки не создают
этот файл из блока и сохраняют пользовательские настройки Pi.

Записи жизненного цикла Pi RPC нормализуются в ограниченные наблюдения
`assistant.diagnostic` с кодами `pi.agent.*`, `pi.turn.started`,
`pi.message.*`, `pi.stream.started` и `pi.auto_retry.*`. В наблюдениях
сохраняются порядковые номера вызова модели и повтора, задержка и ошибка
повтора, статистика использования сообщения, а также наблюдаемые клиентом
`wait_ms`, `stream_ms` и `total_ms`. Эти длительности разделяют локальное
ожидание и передачу потока, но не выдаются за время очереди провайдера или
