...
| `HOST-002` | Зафиксировать и проверить реально поддерживаемую версию Pi | integration owner | Выбрана pinned версия Pi и повторён полный conformance; текущее сохранённое evidence относится к Pi `0.83.0`, локально наблюдается Pi `0.84.1` | credentials и live Pi | открыто |
...
| ID | Задача | Владелец | Критерий закрытия | Зависимости | Статус |
|---|---|---|---|---|---|
| `EVAL-001` | Измерить полезность Takt на реальном обезличенном corpus через Pi | product/evaluation owner | После `FLOW-001`: для Route DSL, Go и Document на pinned Pi — штатный validator, минимум 3 repeat на case/strategy, `success@1`, final success, attempts-to-valid, time-to-valid, tokens/cost, stable/unstable cases и manual corrections; отчёты сохранены с model/adapter/version fingerprints | `FLOW-001`, внешний corpus, credentials, модели | открыто |
...
| `FLOW-002` | static approval reject/revise path | use case нельзя выразить существующими `loop_group` + approval + `when` без потери governance | fail-closed schema/runtime semantics и E2E contract |
...
| `ADAPTER-001` | Углубление OpenCode/Qwen и прочих non-Pi adapters | завершён Pi-first flow gate и появился конкретный assistant/provider use case | отдельные credentialed smoke с redacted evidence; fake fixtures не считаются production proof |
...
1. Зафиксировать Pi и закрыть `FLOW-001` вместе с `HOST-002`.
