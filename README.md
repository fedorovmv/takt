...
`v0.1.38-alpha` предназначена для **локального однопользовательского trusted runtime**. Workflow, config, Markdown-команды и рабочая директория считаются доверенными.
...
- профиль `code` 0.13.0 с 19 процессами разработки, Task Router и девятью встроенными доверенными блоками и шестью глубокими workflow со строгими JSON-входами, checkpoint artifacts, domain errors, Git/recovery semantics;
