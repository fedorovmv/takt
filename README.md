...
`v0.1.51-alpha` предназначена для **локального однопользовательского trusted runtime**. Workflow, config, Markdown-команды и рабочая директория считаются доверенными.
...
К `v0.1.51-alpha` основные механизмы локального Takt уже собраны. Ближайшая цель — **доказать пользу существующих стратегий на реальных задачах и завершить стабилизацию v0.2**, а не продолжать наращивать runtime без измерений.
...
5. P2 External seams продолжен в `v0.1.50`: reference Qwen/GitHub SCM и Structured Task Sources реализованы поверх public SDK; из крупного P2 остаётся live host conformance;
6. P3 открыт в `v0.1.51`: human-reviewed Skill/Block Learning Loop реализован без автоматической мутации trusted packages; следующим UX-срезом остаются graph/explain/scaffold и bounded reject/revise.
